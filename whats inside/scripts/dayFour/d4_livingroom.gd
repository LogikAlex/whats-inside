...
@onready var tvSound: AudioStreamPlayer2D = $tv_sound
...
func sitOnCouch():
	...
	watch.tween_property(tvSound, "playing", true, 0).set_delay(4)
	...
	watch.tween_property(tvSound, "volume_db", -20, 0).set_delay(11)
	watch.tween_property(blackScreen, "modulate:a", 1, 3).set_delay(11)
	watch.tween_callback(
	func changeScene():
		get_tree().change_scene_to_file("res://scenes/Days/Tweaking Day/starter_bedroom.tscn")
	).set_delay(15)
