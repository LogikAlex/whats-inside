...
func camTween():
	...
	cam_rotate.tween_property(camera, "rotation_degrees", 38.2, 3).set_ease(Tween.EASE_IN_OUT)
	cam_rotate.tween_property(camera, "position", Vector2(-67.0, -145.0), 3).set_ease(Tween.EASE_IN_OUT)
	cam_rotate.tween_property(camera, "rotation_degrees", -38.2, 3).set_delay(3).set_ease(Tween.EASE_IN_OUT)
